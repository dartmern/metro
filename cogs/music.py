...
class Song:
    ...
    def create_embed(self):
        ...
        embed.description = (
                f"```\n{self.source.title}\n```"
                f'\n:artist: **Artist:** [`{self.source.uploader}`]({self.source.uploader_url}) **|** [`Video URL`]({self.source.url})'
                f'\n⏲️ **Duration:** `{self.source.duration}`'
                f'\n<:members:908483589157576714> **Requested by:** {self.requester.mention}'
                f'\n\👀 **Total Views:** `{humanize.intword(self.source.views)}`'
                f'\n\👍 **Likes:** `{humanize.intword(self.source.likes)}`'
                f'\n\👎 **Dislikes:** `{humanize.intword(self.source.dislikes)}`'
        )
        embed.colour = discord.Colour.blue()
        embed.set_thumbnail(url=self.source.thumbnail)
        embed.set_footer(text=f'Requested by {self.requester} (In streaming)', icon_url=self.requester.avatar.url)
...
class music(commands.Cog, description=':musical_note: Make your server enjoy music within discord!'):
    ...
    @commands.command(help="Make me join in a VC", name ="join", invoke_without_subcommand=True)
    ...
    @commands.command(help="Summon me in a VC", name ="summon")
...
# Search whatever u want on youtube!
    ...
    async def _volume(self, ctx: commands.Context, *, volume : Union[int, str]):
        ...
        if isinstance(volume, str):
            if volume.lower() == "reset":
                ...
                em = discord.Embed(title=f"Volume set to **`100%`**", color = ctx.author.color)
                em.set_footer(text=f"Regulated by {ctx.author.name}")    
                await ctx.send(embed=em)
            else:
                raise commands.BadArgument("Invaild input for `volume` Input a integer or **reset**")

        if isinstance(volume, int):
            if volume >= 150 or volume <= 0:
                raise commands.BadArgument('Volume must be in between 1-150.')
    ...
    @commands.command(help="See the actual song in playing", aliases=['current', 'playing'], name='now')
